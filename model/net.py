...
class AdversaryPush_CNNPolicy(nn.Module):
    def __init__(self, frames, action_space):
        super(AdversaryPush_CNNPolicy, self).__init__()
        self.logstd = nn.Parameter(torch.zeros(action_space))
        ...
        self.act_fc2 =  nn.Linear(2+2+2+1, 128)# nn.Linear(256+2+2, 128)
        self.actor1 = nn.Linear(128, 1)
        self.actor2 = nn.Linear(128, 1)
        ...
        self.crt_fc2 = nn.Linear(2+2+2+1, 128)#nn.Linear(256+2+2, 128)
        self.critic = nn.Linear(128, 1)



    def forward(self, goal, speed):
        """
            returns value estimation, action, log_action_prob
        """
        # action
    ...
    def evaluate_actions(self, goal, speed, action):
        v, _, _, mean = self.forward(goal, speed)
        logstd = self.logstd.expand_as(mean)
        std = torch.exp(logstd)
        # evaluate
        logprob = log_normal_density(action, mean, log_std=logstd, std=std)
