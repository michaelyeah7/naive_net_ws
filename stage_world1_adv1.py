...
class StageWorld():
    def __init__(self, beam_num, index, num_env):
        ...
        # pro_crash_topic = 'robot_' + str(index) + '/is_pro_crashed'
        # self.check_pro_crashed = rospy.Publisher()
    ...
    def control_pose(self, pose):
        # turState = ModelState()
        # turState.model_name = "robot_" + str(self.index)
    ...
    def generate_random_pose(self):
        ...
        dis = np.sqrt(x ** 2 + y ** 2)
        while (dis > 4) and not rospy.is_shutdown():
            x = np.random.uniform(-4, 4)
            y = np.random.uniform(-4, 4)
            dis = np.sqrt(x ** 2 + y ** 2)
    ...
    def generate_random_goal(self):
        ...
        while (dis_origin > 4 or dis_goal > 5 or dis_goal < 4) and not rospy.is_shutdown():
            x = np.random.uniform(-4, 4)
            y = np.random.uniform(-4, 4)
            dis_origin = np.sqrt(x ** 2 + y ** 2)
            dis_goal = np.sqrt((x - self.init_pose[0]) ** 2 + (y - self.init_pose[1]) ** 2)
