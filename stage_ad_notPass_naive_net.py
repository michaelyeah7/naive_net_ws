...
class StageWorld():
    def __init__(self, beam_num, index, num_env,ros_port,mpi_rank,env_index,goal_robotIndex):
        ...
        crash_topic = 'robot_1' + '/is_crashed'
        ...
        agent_state_topic = 'robot_0' + '/base_pose_ground_truth'
        self.agent_state_sub = rospy.Subscriber(agent_state_topic, Odometry, self.agent_ground_truth_callback)

        agent_goal_topic = 'robot_0' + '/goal_pose'
        self.agent_goal_sub = rospy.Subscriber(agent_goal_topic, Pose, self.agent_goal_topic_callback)
        #self.goal_point = [0,0]
    ...
    def agent_ground_truth_callback(self, GT_odometry):
        Quaternious = GT_odometry.pose.pose.orientation
        Euler = tf.transformations.euler_from_quaternion([Quaternious.x, Quaternious.y, Quaternious.z, Quaternious.w])
        self.agent_state_GT = [GT_odometry.pose.pose.position.x, GT_odometry.pose.pose.position.y, Euler[2]]       

    def agent_goal_topic_callback(self, goal):
        self.agent_goal = [goal.position.x, goal.position.y]
    ...
    def get_agent_stateGT(self):
        return self.agent_state_GT
    ...
    def get_local_ag_pos_and_goal(self):
        [x, y, theta] = self.get_self_stateGT()
        #[agent_goal_x, agent_goal_y] = self.get_agent_goal()
        [agent_goal_x, agent_goal_y] = [0,3]
        #[ag_x, ag_y, ag_vx, ag_vy, ag_theta] = self.get_agent_stateGT()
        [ag_x, ag_y, ag_theta] = self.get_agent_stateGT()
        local_ag_x = (ag_x - x) * np.cos(theta) + (ag_y - y) * np.sin(theta)
        local_ag_y = -(ag_x - x) * np.sin(theta) + (ag_y - y) * np.cos(theta)
        local_agent_goal_x = (agent_goal_x - x) * np.cos(theta) + (agent_goal_y - y) * np.sin(theta)
        local_agent_goal_y = -(agent_goal_x - x) * np.sin(theta) + (agent_goal_y - y) * np.cos(theta)

        agent_pos_goal_distance = np.sqrt((agent_goal_x - ag_x) ** 2 + (agent_goal_y - ag_y) ** 2)
        return [local_ag_x, local_ag_y, local_agent_goal_x, local_agent_goal_y, agent_pos_goal_distance]
    ...
    def get_reward_and_terminate(self, t):
        ...
        agent_goal_x = 0.0
        agent_goal_y = 3.0
        is_agent_reach_goal = False
        reward_lose = 0
        agent_pos_goal_distance = np.sqrt((agent_goal_x - ag_x) ** 2 + (agent_goal_y - ag_y) ** 2)
        if agent_pos_goal_distance < 0.5:
            is_agent_reach_goal = True
        # if is_crash == 1:
        ...
        if is_agent_reach_goal == True:
            terminate = True
            reward_lose = -15
            result = "agent reached the goal, adversary failed"
